// SPDX-FileCopyrightText: 2014 Silicon Laboratories Inc. <https://www.silabs.com/>
...
#if defined(DEBUG) && defined(DEBUGPRINT)
...
next_mpan_state(struct MPAN * mpan)
{
  ...
  bigint_add_big_endian(mpan->inner_state, mpan->inner_state, one, 16);
}
  ...
  *plain_text_len = 0;
  ...
  else
  {
    ...
    decrypt_len = CCM_decrypt_and_auth(ctxt->sg[mpan->class_id].enc_key, nonce, aad, aad_len, ciphertext,
        ciphertext_len);
    /**
     * If authentication or decryption fails, try one more time.
     * According to requirement CC:009F.01.00.12.00A,
      ...
      if (plain_text_len)
      {
            ...
            !(plain_text[1] == SECURITY_2_COMMANDS_SUPPORTED_REPORT))
        {
          ...
          if (plain_text[1] == SECURITY_2_COMMANDS_SUPPORTED_GET)
          {
            ...
            if ((size_t)(n_commands_supported + 2) > sizeof(ctxt->u.commands_sup_report_buf))
            {
