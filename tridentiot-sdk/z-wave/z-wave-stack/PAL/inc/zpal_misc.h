...
/**
 * @brief Get the Hardware Manufacturer ID
 *
 * @return the Hardware Manufacturer ID
 */
uint16_t zpal_get_hardware_manufacturer_id(void);

/**
 * @brief Get the Manufacturer Specific Chip Info
 *
 * If the Manufacturer Specific Chip Info is not available, the function will
 * provide an info length of zero.
 *
 * It's up to the ZPAL Manufacturer to
 * fill the fields with info that can be used to distinguish the chip from
 * others in their product catalog. Additionally, they should provide
 * documentation describing the mapping and meaning of these fields.
 *
 * Example information that could be included:
 *    First byte is the chip generation.
 *    Second byte is the chip type (module or SoC).
 *    Third byte is the chip SKU.
 *    Fourth byte is the chip revision.
 *
 * @param[out] p_info Address of array to where the Manufacturer Specific Chip Info will be written.
 * @param[out] p_info_length Length in bytes of the Manufacturer Specific Chip Info.
 * @param[in]  info_length_max Size of the array provided to p_info.
 *
 * @return Returns false if info_length_max is smaller than the required memory, otherwise true.
 */
