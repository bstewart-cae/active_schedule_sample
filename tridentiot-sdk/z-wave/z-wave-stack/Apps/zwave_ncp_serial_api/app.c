  ...
  )
{
  ...
  comm_interface_transmit_frame(cmd, RESPONSE, pData, len, NULL, NULL);

  set_state_and_notify(stateTxSerial); /* We want ACK/NAK...*/
}

void Respond_with_ack_cb(
  uint8_t cmd,
  uint8_t const *pData,
  uint8_t len,
  ...
  )
{
  ...
  comm_interface_transmit_frame(cmd, RESPONSE, pData, len, NULL, ack_cb);
...
void DoRespond_workbuf_with_ack_cb(uint8_t cnt, ack_received_cb_t ack_cb)
{
  Respond_with_ack_cb(serial_frame->cmd, compl_workbuf, cnt, ack_cb);
}
      ...
      case stateTxSerial:
      {
        ...
        if ((conVal = comm_interface_parse_data(false)) == PARSE_FRAME_SENT) {
          ...
          if (retry++ < MAX_SERIAL_RETRY) {
            comm_interface_transmit_frame(0, REQUEST, NULL, 0, NULL, NULL); /* Retry... */
          } else {
...
#ifdef DEBUGPRINT
  static uint8_t m_aDebugPrintBuffer[250];
...
ZW_WEAK void SerialAPI_get_uart_config(serialapi_uart_config_t *uart_config)
{
  ASSERT(uart_config != NULL);
  if (NULL != uart_config)
  {
    uart_config->uart_id    = ZPAL_UART1;
