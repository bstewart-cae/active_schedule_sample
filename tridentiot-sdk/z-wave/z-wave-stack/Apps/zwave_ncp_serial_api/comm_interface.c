...
typedef struct
{
  ...
  ack_received_cb_t ack_received_cb;
} comm_interface_t;
...
typedef struct
{
  uint32_t sof_hunting_dropped_bytes;
  uint32_t len_invalid_dropped_frames;
  uint32_t type_invalid_dropped_frames;
  uint32_t checksum_invalid_dropped_frames;
} frame_handler_statistics_t;

static frame_handler_statistics_t frame_handler_statistics = {
  .sof_hunting_dropped_bytes = 0,
  .len_invalid_dropped_frames = 0,
  .type_invalid_dropped_frames = 0,
  .checksum_invalid_dropped_frames = 0,
};
...
void comm_interface_transmit_frame(
  uint8_t cmd,
  uint8_t type,
  const uint8_t *payload,
  uint8_t len,
  transmit_done_cb_t cb,
  ack_received_cb_t ack_cb)
{
  tx_frame_t frame = {
    .sof = SOF
  };
  ...
  comm_interface.ack_received_cb = ack_cb;
...
{
  serialapi_uart_config_t serialapi_uart;

  SerialAPI_get_uart_config(&serialapi_uart);
  ...
  const zpal_uart_config_t uart_config =
  {
    ...
    .id = serialapi_uart.uart_id,
    .baud_rate = serialapi_uart.baud_rate,
    .data_bits = serialapi_uart.data_bits,
    .parity_bit = serialapi_uart.parity_bit,
    .stop_bits = serialapi_uart.stop_bits,
...
static comm_interface_parse_result_t handle_sof(uint8_t input)
{
  ...
  if (input == SOF) {
    ...
    if (comm_interface.ack_needed)
    {
      ...
      if (input == ACK) {
        if (NULL != comm_interface.ack_received_cb) {
          comm_interface.ack_received_cb();
          comm_interface.ack_received_cb = NULL;
        }
      ...
      // Bogus character received...
      frame_handler_statistics.sof_hunting_dropped_bytes++;
    }
  }
  return result;
}
...
static void handle_len(uint8_t input)
{
  ...
  {
    frame_handler_statistics.len_invalid_dropped_frames++;
...
static void handle_type(uint8_t input)
{
  if (input > RESPONSE)
  {
    frame_handler_statistics.type_invalid_dropped_frames++;
    comm_interface.state = COMM_INTERFACE_STATE_SOF; // Restart looking for SOF
    comm_interface.rx_active = false;  // Not really active now...
    TimerStop(&comm_interface.byte_timer);
    comm_interface.byte_timeout = false;
...
static comm_interface_parse_result_t handle_checksum(uint8_t input, bool ack)
{
  ...
  if (ack)
  {
    ...
    if (result == PARSE_FRAME_ERROR)
    {
      frame_handler_statistics.checksum_invalid_dropped_frames++;
    }
  }
