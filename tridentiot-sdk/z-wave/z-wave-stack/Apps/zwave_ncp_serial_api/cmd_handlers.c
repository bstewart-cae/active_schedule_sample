...
static void Initiate_shutdown_cb(void)
{
  ...
  comm_interface_transmit_frame(FUNC_ID_ZW_INITIATE_SHUTDOWN, RESPONSE, &status, sizeof(status), NULL, NULL);
...
ZW_ADD_CMD(FUNC_ID_ZW_SET_PRIORITY_ROUTE)
{
  ...
  uint8_t  offset = 0;
  node_id_t nodeId = (node_id_t)GET_NODEID(&frame->payload[0], offset);
  uint8_t nodeIdSize = (SERIAL_API_SETUP_NODEID_BASE_TYPE_16_BIT == nodeIdBaseType) ? 2 : 1;
  if (SERIAL_API_SETUP_NODEID_BASE_TYPE_16_BIT == nodeIdBaseType)
  {
    BYTE_IN_AR(compl_workbuf, 0) = (uint8_t)(nodeId >> 8);     // MSB
    BYTE_IN_AR(compl_workbuf, 1) = (uint8_t)(nodeId & 0xFF);   // LSB
  ...
  if ((offset + 9) <= frame->len)
  {
    ...
    BYTE_IN_AR(compl_workbuf, nodeIdSize) = SetPriorityRoute(nodeId, &frame->payload[nodeIdSize]);
  }
  else
  {
    /* Clear/Release Golden Route - Devkit 6.6x+ */
    BYTE_IN_AR(compl_workbuf, nodeIdSize) = SetPriorityRoute(nodeId, NULL);
  }
  DoRespond_workbuf(nodeIdSize + 1);
}
#endif
...
ZW_ADD_CMD(FUNC_ID_ZW_GET_VERSION)
{
  ...
  volatile int32_t iCharacters = snprintf((char *)(&compl_workbuf[0]), 12, "Z-Wave %1d.%02d", 0, 0);
...
ZW_ADD_CMD(FUNC_ID_ZW_GET_MANUFACTURER_INFO)
{
  const uint8_t INFO_LENGTH_MAX = 16; // 16 is an arbitrary chosen number.

  STATIC_ASSERT(sizeof(compl_workbuf) >= (6 + 16), STATIC_ASSERT_compl_workbuf_manufacturer_info_size);

  const uint16_t hwManufacturerId = zpal_get_hardware_manufacturer_id();
  const uint16_t protoManufacturerId = ZW_GetProtocolManufacturerId();

  uint8_t i = 0;
  compl_workbuf[i++] = (uint8_t)((hwManufacturerId >> 8) & 0xFF);
  compl_workbuf[i++] = (uint8_t)((hwManufacturerId >> 0) & 0xFF);

  compl_workbuf[i++] = (uint8_t)((protoManufacturerId >> 8) & 0xFF);
  compl_workbuf[i++] = (uint8_t)((protoManufacturerId >> 0) & 0xFF);

  compl_workbuf[i++] = (uint8_t)((ZAF_CONFIG_MANUFACTURER_ID >> 8) & 0xFF);
  compl_workbuf[i++] = (uint8_t)((ZAF_CONFIG_MANUFACTURER_ID >> 0) & 0xFF);

  // Reserve a field for the info length and save the index.
  uint8_t info_length_index = i++;

  uint8_t info_length = 0;
  zpal_get_manufacturer_specific_chip_info(&compl_workbuf[i], &info_length, INFO_LENGTH_MAX);
  i += info_length;

  compl_workbuf[info_length_index] = info_length;
