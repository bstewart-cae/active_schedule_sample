...
#define CAP_BM(OFFSET) (CAP_FID(OFFSET, (SUPPORT_SERIAL_API_POWER_MANAGEMENT == 0) ? 0 : FUNC_ID_SERIAL_API_POWER_MANAGEMENT) | \
                        ...
                        CAP_FID(OFFSET, (SUPPORT_MEMORY_PUT_BUFFER == 0) ? 0 : FUNC_ID_MEMORY_PUT_BUFFER) | \
                        CAP_FID(OFFSET, (SUPPORT_NVM_BACKUP_RESTORE == 0) ? 0 : FUNC_ID_NVM_BACKUP_RESTORE) | \
                        CAP_FID(OFFSET, (SUPPORT_NVM_EXT_BACKUP_RESTORE == 0) ? 0 : FUNC_ID_NVM_EXT_BACKUP_RESTORE) | \
                        CAP_FID(OFFSET, (SUPPORT_NVM_GET_ID == 0) ? 0 : FUNC_ID_NVM_GET_ID) | \
                        CAP_FID(OFFSET, (SUPPORT_NVM_EXT_READ_LONG_BYTE == 0) ? 0 : FUNC_ID_NVM_EXT_READ_LONG_BYTE) | \
                        CAP_FID(OFFSET, (SUPPORT_NVM_EXT_WRITE_LONG_BYTE == 0) ? 0 : FUNC_ID_NVM_EXT_WRITE_LONG_BYTE) | \
                        CAP_FID(OFFSET, (SUPPORT_NVM_EXT_READ_LONG_BUFFER == 0) ? 0 : FUNC_ID_NVM_EXT_READ_LONG_BUFFER) | \
...
/**
 * @brief Responds with the content of @ref compl_workbuf and invokes @p ack_cb
 *        on reception of ack.
 *
 * @param cnt Number of bytes in @ref compl_workbuf to transmit.
 * @param ack_cb Callback to invoke upon reception of ack.
 */
extern void DoRespond_workbuf_with_ack_cb(uint8_t cnt, ack_received_cb_t ack_cb);
...
/**
 * @brief Sends a response and invokes a given callback upon ack reception.
 *
 * @param cmd Serial API command ID
 * @param pData Pointer to payload.
 * @param len Length of payload.
 * @param ack_cb Callback to invoke upon reception of ack.
 */
extern void Respond_with_ack_cb(
  uint8_t cmd,
  uint8_t const *pData,
  uint8_t len,
...
/**
 * Used for frames send to remote side.
 */
