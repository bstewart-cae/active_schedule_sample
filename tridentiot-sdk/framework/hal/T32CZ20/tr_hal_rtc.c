...
/// @file tr_hal_rtc.c
///
/// @brief This contains the code for the Trident HAL RTC for T32CZ20
///
///
...
// keep track of if the RTC is running
bool g_is_rtc_running = false;
...
uint32_t convert_bcd_to_int(uint32_t register_value)
{
    ...
    uint32_t tens = (register_value >> 4) & 0x0F;
    uint32_t hundreds = (register_value >> 8) & 0x0F;
    uint32_t return_value = ones + (tens * 10) + (hundreds * 100);
...
tr_hal_status_t tr_hal_rtc_init(tr_hal_rtc_settings_t* rtc_settings)
{
    ...
    if ( (rtc_settings->combo_event_enabled)
        &&
          (  (rtc_settings->milliseconds_event_trigger != TR_HAL_EVENT_TRIGGER_OFF)
    ...
    {
        // milliseconds
        status = tr_hal_rtc_set_time_trigger_event(TR_HAL_RTC_TIME_UNIT_MILLISECONDS,
                                                   rtc_settings->milliseconds_event_trigger,
                                                   rtc_settings->milliseconds_trigger_value);
        if (status != TR_HAL_SUCCESS)
        {
            g_rtc_ok_to_run_load = true;
            return status;
    ...
    // and that the RTC is enabled (the enable bit turns the RTC ON)
    rtc_reg->control = TR_HAL_RTC_CONTROL_REG_ENABLE_BIT | TR_HAL_RTC_SPEEDUP_NONE;
...
{
    // on the CZ20 some code needs to start the RTC
    (*is_running) = g_is_rtc_running;
...
tr_hal_status_t tr_hal_rtc_get_time(tr_hal_rtc_time* return_time)
{
    ...
    uint32_t milliseconds_register_value = rtc_reg->milliseconds_value;
    ...
    return_time->milliseconds = (uint16_t) convert_bcd_to_int(milliseconds_register_value);
...
{
    uint32_t larger_seconds = (uint32_t)((time1->hours * 3600) + (time1->minutes * 60) + time1->seconds);
    uint32_t smaller_seconds = (uint32_t)((time2->hours * 3600) + (time2->minutes * 60) + time2->seconds);

    uint32_t larger_milliseconds = (uint32_t)((larger_seconds * 1000) + time1->milliseconds);
    uint32_t smaller_milliseconds = (uint32_t)((smaller_seconds * 1000) + time2->milliseconds);

    // if they are equal then result is false
    if (smaller_milliseconds == larger_milliseconds)
    {
        return false;
    }
    ...
    if (smaller_milliseconds > larger_milliseconds)
    {
        diff = smaller_milliseconds - larger_milliseconds;
    }
    else
    {
        diff = larger_milliseconds - smaller_milliseconds;
    }
                                                  ...
                                                  uint16_t                   trigger_value)
{
    ...
    if (g_combo_event_active)
    {
        ...
        rtc_reg->milliseconds_event_trigger = TR_HAL_REG_SETTING_EVENT_OFF;
    ...
    {
        // *** milliseconds ***
        case TR_HAL_RTC_TIME_UNIT_MILLISECONDS:

            // check range if we are setting a value
            if (   (event_trigger == TR_HAL_EVENT_TRIGGER_ON_SPECIFIC_VALUE)
                && (trigger_value > TR_HAL_RTC_MILLISECOND_MAXIMUM))
            {
                return TR_HAL_RTC_INVALID_MILLISECOND;
            }
            // milliseconds trigger = OFF
            if (event_trigger == TR_HAL_EVENT_TRIGGER_OFF)
            {
                // set event-trigger register to out-of-range value and turn off interrupt,
                rtc_reg->milliseconds_event_trigger = TR_HAL_REG_SETTING_EVENT_OFF | TR_HAL_RTC_MILLISECOND_TRIGGER_OFF_VALUE;
                rtc_reg->interrupt_enable &= (uint32_t)(~TR_HAL_RTC_INTERRUPT_MILLISECONDS);
            }
            // milliseconds trigger = ON CHANGE
            else if (event_trigger == TR_HAL_EVENT_TRIGGER_ON_UNIT_CHANGE)
            {
                // set the register and turn on the interrupt
                rtc_reg->milliseconds_event_trigger = TR_HAL_REG_SETTING_EVENT_ON_CHANGE;
                rtc_reg->interrupt_enable |= TR_HAL_RTC_INTERRUPT_MILLISECONDS;
            }
            ...
            else if (event_trigger == TR_HAL_EVENT_TRIGGER_ON_SPECIFIC_VALUE)
            {
                ...
                rtc_reg->milliseconds_event_trigger = TR_HAL_REG_SETTING_EVENT_ON_VALUE | bcd_trigger_value;
                rtc_reg->interrupt_enable |= TR_HAL_RTC_INTERRUPT_MILLISECONDS;
            }
            break;
        ...
        case TR_HAL_RTC_TIME_UNIT_SECONDS:
            ...
            {
                // set event-trigger register to out-of-range value and turn off interrupt,
                rtc_reg->seconds_event_trigger = TR_HAL_REG_SETTING_EVENT_OFF | TR_HAL_RTC_SECOND_TRIGGER_OFF_VALUE;
                rtc_reg->interrupt_enable &= (uint32_t)(~TR_HAL_RTC_INTERRUPT_SECONDS);
            }
        ...
        case TR_HAL_RTC_TIME_UNIT_MINUTES:
            ...
            {
                // set event-trigger register to out-of-range value and turn off interrupt,
                rtc_reg->minutes_event_trigger = TR_HAL_REG_SETTING_EVENT_OFF | TR_HAL_RTC_MINUTE_TRIGGER_OFF_VALUE;
                rtc_reg->interrupt_enable &= (uint32_t)(~TR_HAL_RTC_INTERRUPT_MINUTES);
            }
        ...
        case TR_HAL_RTC_TIME_UNIT_HOURS:
            ...
            {
                // set event-trigger register to out-of-range value and turn off interrupt,
                rtc_reg->hours_event_trigger = TR_HAL_REG_SETTING_EVENT_OFF | TR_HAL_RTC_HOUR_TRIGGER_OFF_VALUE;
                rtc_reg->interrupt_enable &= (uint32_t)(~TR_HAL_RTC_INTERRUPT_HOURS);
            }
        ...
        case TR_HAL_RTC_TIME_UNIT_DAYS:
            ...
            {
                // set event-trigger register to out-of-range value and turn off interrupt,
                rtc_reg->days_event_trigger = TR_HAL_REG_SETTING_EVENT_OFF | TR_HAL_RTC_DAY_TRIGGER_OFF_VALUE;
                rtc_reg->interrupt_enable &= (uint32_t)(~TR_HAL_RTC_INTERRUPT_DAYS);
            }
        ...
        case TR_HAL_RTC_TIME_UNIT_MONTHS:
            ...
            {
                // set event-trigger register to out-of-range value and turn off interrupt,
                rtc_reg->months_event_trigger = TR_HAL_REG_SETTING_EVENT_OFF | TR_HAL_RTC_MONTH_TRIGGER_OFF_VALUE;
                rtc_reg->interrupt_enable &= (uint32_t)(~TR_HAL_RTC_INTERRUPT_MONTHS);
            }
        ...
        case TR_HAL_RTC_TIME_UNIT_YEARS:
            ...
            {
                // set event-trigger register to out-of-range value and turn off interrupt,
                rtc_reg->years_event_trigger = TR_HAL_REG_SETTING_EVENT_OFF | TR_HAL_RTC_YEAR_TRIGGER_OFF_VALUE;
                rtc_reg->interrupt_enable &= (uint32_t)(~TR_HAL_RTC_INTERRUPT_YEARS);
            }
...
tr_hal_status_t tr_hal_rtc_set_combo_trigger_event(tr_hal_rtc_date_time new_date_time)
{
    ...
    // ms set to 0
    rtc_reg->milliseconds_event_trigger = 0;
    ...
    // so only enable the combo event interrupt
...
void RTC_Handler(void)
{
    ...
    // we sometimes get interrupts that we did not enable. To prevent getting
    // undesired events we filter here on what we have enabled
    uint32_t interrupt_enabled_mask = rtc_reg->interrupt_enable;
    uint32_t interrupt_to_service = interrupt_triggered_mask & interrupt_enabled_mask;

    // milliseconds
    if (interrupt_to_service & TR_HAL_RTC_INTERRUPT_MILLISECONDS)
    {
        event_bitmask |= TR_HAL_RTC_EVENT_TRIGGERED_MILLISECONDS;
    }
    // seconds
    if (interrupt_to_service & TR_HAL_RTC_INTERRUPT_SECONDS)
    {
        event_bitmask |= TR_HAL_RTC_EVENT_TRIGGERED_SECONDS;
    }
    // minutes
    if (interrupt_to_service & TR_HAL_RTC_INTERRUPT_MINUTES)
    {
        event_bitmask |= TR_HAL_RTC_EVENT_TRIGGERED_MINUTES;
    }
    // hours
    if (interrupt_to_service & TR_HAL_RTC_INTERRUPT_HOURS)
    {
        event_bitmask |= TR_HAL_RTC_EVENT_TRIGGERED_HOURS;
    }
    // days
    if (interrupt_to_service & TR_HAL_RTC_INTERRUPT_DAYS)
    {
        event_bitmask |= TR_HAL_RTC_EVENT_TRIGGERED_DAYS;
    }
    // months
    if (interrupt_to_service & TR_HAL_RTC_INTERRUPT_MONTHS)
    {
        event_bitmask |= TR_HAL_RTC_EVENT_TRIGGERED_MONTHS;
    }
    // years
    if (interrupt_to_service & TR_HAL_RTC_INTERRUPT_YEARS)
    {
        event_bitmask |= TR_HAL_RTC_EVENT_TRIGGERED_YEARS;
    }
    // combined event
    if (interrupt_to_service & TR_HAL_RTC_INTERRUPT_COMBINED_EVENT)
    {
        event_bitmask |= TR_HAL_RTC_EVENT_TRIGGERED_COMBINED_EVENT;
    }

    // clear interrupt state - use the original mask and not the modified one
...
tr_hal_status_t tr_hal_rtc_speedup_for_testing(tr_hal_rtc_speedup_unit_t speedup_unit)
{
    ...
    rtc_reg->control = TR_HAL_RTC_CONTROL_REG_ENABLE_BIT | speedup_unit;
