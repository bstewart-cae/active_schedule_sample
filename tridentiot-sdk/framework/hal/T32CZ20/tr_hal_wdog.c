...
/// @file tr_hal_wdog.c
///
/// @brief This contains the code for the Trident HAL Watchdog for T32CZ20
///
///
...
static tr_hal_status_t check_init_params(tr_hal_wdog_settings_t* wdog_settings)
{
    ...
    uint32_t prescalar_raw = wdog_settings->clock_prescalar;
    if (prescalar_raw > TR_HAL_WDOG_PRESCALAR_MAX_VALUE)
    {
    ...
    uint32_t prescalar = (wdog_settings->clock_prescalar) + 1;
...
tr_hal_status_t tr_hal_wdog_init(tr_hal_wdog_settings_t* wdog_settings)
{
    ...
    // set clock prescalar
    wdog_reg->clock_prescale = wdog_settings->clock_prescalar;
    ...
    // Start timer if watchdog should be enabled on init
...
tr_hal_status_t tr_hal_wdog_enable(void)
{
    ...
    // value and force the timer enable bit.
    // Note: If the lock bit is set then the watchdog configuration will be
    // locked now
...
tr_hal_status_t tr_hal_wdog_read_curr_state(tr_hal_wdog_settings_t* wdog_settings)
{
    ...
    wdog_settings->clock_prescalar = wdog_reg->clock_prescale;
