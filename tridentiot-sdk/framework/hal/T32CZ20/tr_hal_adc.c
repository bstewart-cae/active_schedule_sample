...
/// @file tr_hal_adc.c
///
/// @brief This contains the code for the Trident HAL ADC for T32CZ20
///
///
                                                ...
                                                tr_hal_adc_settings_t* adc_settings)
{
    ...
    if ( (pin_num != ADC_VALID_PIN_CHOICE1)
        ...
        && (pin_num != ADC_VALID_PIN_CHOICE5)
        && (pin_num != ADC_VALID_PIN_CHOICE6)
        && (pin_num != ADC_VALID_PIN_CHOICE7)
       )
    {
...
{
    ADC_REGISTERS->analog_settings0.bit.AUX_ADC_DEBUG = 0;
    ADC_REGISTERS->analog_settings0.bit.AUX_ADC_MODE = 0;
    ADC_REGISTERS->analog_settings0.bit.AUX_ADC_OUTPUTSTB = 0;
    ADC_REGISTERS->analog_settings0.bit.AUX_ADC_OSPN = 0;
    ADC_REGISTERS->analog_settings0.bit.AUX_ADC_CLK_SEL = 0;
    ADC_REGISTERS->analog_settings0.bit.AUX_ADC_MCAP = 3;
    ADC_REGISTERS->analog_settings0.bit.AUX_ADC_MDLY = 2;
    ADC_REGISTERS->analog_settings0.bit.AUX_ADC_SEL_DUTY = 2;
    ADC_REGISTERS->analog_settings0.bit.AUX_ADC_OS = 0;
    ADC_REGISTERS->analog_settings0.bit.AUX_ADC_BR = 15;
    ADC_REGISTERS->analog_settings0.bit.AUX_ADC_PW = 0;
    ADC_REGISTERS->analog_settings0.bit.AUX_ADC_STB_BIT = 0;
    ADC_REGISTERS->analog_settings0.bit.AUX_PW = 2;

    ADC_REGISTERS->analog_settings1.bit.AUX_VGA_CMSEL = 0;
    ADC_REGISTERS->analog_settings1.bit.AUX_VGA_COMP = 1;
    ADC_REGISTERS->analog_settings1.bit.AUX_VGA_SIN = 0;
    ADC_REGISTERS->analog_settings1.bit.AUX_VGA_LOUT = 0;
    ADC_REGISTERS->analog_settings1.bit.AUX_VGA_SW_VDD = 0;
    ADC_REGISTERS->analog_settings1.bit.AUX_VGA_VLDO = 2;
    ADC_REGISTERS->analog_settings1.bit.AUX_VGA_ACM = 15;
    ADC_REGISTERS->analog_settings1.bit.AUX_VGA_PW = 31;
    ADC_REGISTERS->analog_settings1.bit.AUX_DC_ADJ = 3;
    ADC_REGISTERS->analog_settings1.bit.AUX_TEST_MODE = 0;
    ADC_REGISTERS->analog_settings1.bit.CFG_EN_CLKAUX = 1;
    ADC_REGISTERS->analog_settings1.bit.AUX_VGA_TEST_AIO_EN = 0;

    AUX_COMP_REGISTERS->COMP_ANA_CTRL.bit.COMP_EN_START = 1;
    // does this matter for ADC? tried and it had no affect I could see
    //AUX_COMP_REGISTERS->COMP_ANA_CTRL.bit.COMP_CHSEL = 1;
}

/// ****************************************************************************
/// INTERNAL FUNCTION tr_hal_analog_pin_enable
///
/// on CZ20: GPIO 21 to 23, 28 to 31 have ADC ability:
/// we don't have GPIO 24 to 27
///    GPIO21 = AIO0
///    GPIO22 = AIO1
///    GPIO23 = AIO2
///    GPIO28 = AIO4
                                         ...
                                         bool              is_p_pin)
{
    ...
    {
        // pin 21 = AIO 0
        case TR_HAL_ADC_AIO0:
        {
            SYS_CTRL_CHIP_REGISTERS->aio_control |= TR_ADC_ENABLE_AIO0;
            if (is_p_pin)
            {
                return ADC_CONFIG_REG_P_CHANNEL_AIN_0;
            }
            else
            {
                return ADC_CONFIG_REG_N_CHANNEL_AIN_0;
            }
        }

        // pin 22 = AIO 1
        case TR_HAL_ADC_AIO1:
        {
            SYS_CTRL_CHIP_REGISTERS->aio_control |= TR_ADC_ENABLE_AIO1;
            if (is_p_pin)
            {
                return ADC_CONFIG_REG_P_CHANNEL_AIN_1;
            }
            else
            {
                return ADC_CONFIG_REG_N_CHANNEL_AIN_1;
            }
        }

        // pin 23 = AIO 2
        case TR_HAL_ADC_AIO2:
        {
            SYS_CTRL_CHIP_REGISTERS->aio_control |= TR_ADC_ENABLE_AIO2;
            if (is_p_pin)
            {
                return ADC_CONFIG_REG_P_CHANNEL_AIN_2;
            }
            else
            {
                return ADC_CONFIG_REG_N_CHANNEL_AIN_2;
            }
        }

        // pin 24 = AIO 3 = not available

        // pin 28 = AIO 4
        case TR_HAL_ADC_AIO4:
        {
            SYS_CTRL_CHIP_REGISTERS->aio_control |= TR_ADC_ENABLE_AIO4;
            if (is_p_pin)
            {
        ...
        {
            SYS_CTRL_CHIP_REGISTERS->aio_control |= TR_ADC_ENABLE_AIO5;
            if (is_p_pin)
            {
        ...
        {
            SYS_CTRL_CHIP_REGISTERS->aio_control |= TR_ADC_ENABLE_AIO6;
            if (is_p_pin)
            {
        ...
        {
            SYS_CTRL_CHIP_REGISTERS->aio_control |= TR_ADC_ENABLE_AIO7;
            if (is_p_pin)
            {
...
static void tr_hal_analog_pin_disable(tr_hal_gpio_pin_t pin_struct)
{
    ...
    switch (pin_num)
    {
        ...
        case TR_HAL_ADC_AIO0:
        {
            SYS_CTRL_CHIP_REGISTERS->aio_control &= (uint32_t) (~TR_ADC_ENABLE_AIO0);
            return;
        }
        ...
        case TR_HAL_ADC_AIO1:
        {
            SYS_CTRL_CHIP_REGISTERS->aio_control &= (uint32_t) (~TR_ADC_ENABLE_AIO1);
            return;
        }
        ...
        case TR_HAL_ADC_AIO2:
        {
            SYS_CTRL_CHIP_REGISTERS->aio_control &= (uint32_t) (~TR_ADC_ENABLE_AIO2);
            return;
        }
        ...
        case TR_HAL_ADC_AIO4:
        {
            SYS_CTRL_CHIP_REGISTERS->aio_control &= (uint32_t) (~TR_ADC_ENABLE_AIO4);
            return;
        }
        ...
        case TR_HAL_ADC_AIO5:
        {
            SYS_CTRL_CHIP_REGISTERS->aio_control &= (uint32_t) (~TR_ADC_ENABLE_AIO5);
            return;
        }
        ...
        case TR_HAL_ADC_AIO6:
        {
            SYS_CTRL_CHIP_REGISTERS->aio_control &= (uint32_t) (~TR_ADC_ENABLE_AIO6);
            return;
        }
        ...
        case TR_HAL_ADC_AIO7:
        {
            SYS_CTRL_CHIP_REGISTERS->aio_control &= (uint32_t) (~TR_ADC_ENABLE_AIO7);
            return;
        }
    }
}
...
// /// ****************************************************************************
// uint32_t tr_hal_get_gain_reg_setting_from_value(uint32_t gain_value)
// {
                                                      ...
                                                      tr_hal_adc_pull_mode_t pin_n_pull_mode)
{
    ...
    switch (pin_p_pull_mode)
    {
        ...
        case TR_HAL_ADC_PULL_TO_VCC:   result |= ADC_CONFIG_REG_P_CHAN_TO_VDD; break;
        case TR_HAL_ADC_PULL_TO_GND:   result |= ADC_CONFIG_REG_P_CHAN_TO_GND; break;
    ...
    switch (pin_n_pull_mode)
    {
        ...
        case TR_HAL_ADC_PULL_TO_VCC:   result |= ADC_CONFIG_REG_N_CHAN_TO_VDD; break;
        case TR_HAL_ADC_PULL_TO_GND:   result |= ADC_CONFIG_REG_N_CHAN_TO_GND; break;
                                ...
                                tr_hal_adc_settings_t*  adc_settings)
{
    ...
    // don't need this, maybe later if we want to calibrate
    //MpCalAdcInit();
    ...
    // read values from settings struct for clock settings
    ...
    // clock settings for analog test - run by SW and not timer
    ADC_REGISTERS->clock_settings = ADC_REG_TIMER_RATE_DEPENDS_ON_SOFTWARE
                                  | clock_to_use_register
                                  | ADC_REG_TIMER_RISING_EDGE
                                  | clock_divider;

    // enable VGA, LDO, and free clock
    // these are needed for ADC to function
    ADC_REGISTERS->control_enable = ADC_REG_ENABLE_VGA_ENABLE
                                  | ADC_REG_ENABLE_LDO_ENABLE
                                  | ADC_REG_ENABLE_CLK_FREE;

    // need to enable MANUAL_MODE1 and MANUAL_MODE2 for 1-shot
    // without these enabled, it does not work
    ADC_REGISTERS->oversample_settings = ADC_REG_SAMPLE_ENABLE_MANUAL_MODE1
                                       | ADC_REG_SAMPLE_ENABLE_MANUAL_MODE2
                                       | adc_settings->oversample
                                       | adc_settings->resolution;
    ...
    // this setting works much better
    // the cfg_sadc_gain_chx bits are listed as TBD in the documentation
    // on CM11 the 4 nibble was x6 db and the 3rd nibble was x3 db
    // meaning 0x1000 and 0x0200 would be the same. however, 0x0200 works MUCH
    // better than 0x1000 so maybe the meaning is different on CZ20
    uint32_t gain_setting = 0x200;
    ...
    ADC_REGISTERS->ch_x_setting[adc_channel_id].ch_x_threshholds = (adc_settings->threshhold_high << 16)
...
tr_hal_status_t tr_hal_adc_set_for_initial_state_all_disabled(void)
{
    ...
    SYS_CTRL_CHIP_REGISTERS->aio_control = 0;
...
void Sadc_Handler(void)
{
        ...
        && (int_status & TR_HAL_ADC_INTERRUPT_VALID) )
    {
        ...
        // channel 4 result
        else if (int_status & TR_HAL_ADC_INTERRUPT_CHAN_4)
        {
            event_bitmask |= TR_HAL_ADC_EVENT_CH_4_RESULT;
            adc_channel_id = ADC_CHANNEL_4_ID;
            converted_result = convert_ADC_reading_to_voltage(raw_result, ADC_CHANNEL_4_ID);
            g_last_read_value[ADC_CHANNEL_4_ID] = raw_result;
        }
        // channel 5 result
        else if (int_status & TR_HAL_ADC_INTERRUPT_CHAN_5)
        {
            event_bitmask |= TR_HAL_ADC_EVENT_CH_5_RESULT;
            adc_channel_id = ADC_CHANNEL_5_ID;
            converted_result = convert_ADC_reading_to_voltage(raw_result, ADC_CHANNEL_5_ID);
            g_last_read_value[ADC_CHANNEL_5_ID] = raw_result;
        }
        // channel 6 result
        else if (int_status & TR_HAL_ADC_INTERRUPT_CHAN_6)
        {
            event_bitmask |= TR_HAL_ADC_EVENT_CH_6_RESULT;
            adc_channel_id = ADC_CHANNEL_6_ID;
            converted_result = convert_ADC_reading_to_voltage(raw_result, ADC_CHANNEL_6_ID);
            g_last_read_value[ADC_CHANNEL_6_ID] = raw_result;
        }
    }
    ...
    {
        adc_channel_id = TR_HAL_ADC_EVENT_CH_0_RESULT;
    }
