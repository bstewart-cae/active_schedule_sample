...
/// @file T32CZ20_rtc.h
///
/// @brief This is the chip specific include file for T32CZ20 RTC Driver
...
#ifndef T32CZ20_RTC_H_
#define T32CZ20_RTC_H_

#include "tr_hal_platform.h"


/// ****************************************************************************
/// @defgroup tr_hal_rtc_cz20 RTC CZ20
...
/// ******************************************************************
#ifdef RTC_SECURE_EN
    #define CHIP_MEMORY_MAP_RTC_BASE     (0x50004000UL)
#else
...
typedef struct
{
    ...
    // NEW in CZ20
    __IO  uint32_t   milliseconds_value;          // 0x48
    __IO  uint32_t   milliseconds_event_trigger;  // 0x4C
...
// 32.768 KHz = 20.C49C = 0x20C49C = 2147484
//#define TR_HAL_DEFAULT_CLOCK_DIVISOR 0x20C49C
// tuned for my board
...
/// ***************************************************************************
/// we want to be able to set events off, but we really can set events to
/// trigger on change or trigger on value. To turn an event off we set to
/// trigger on value and put an out-of-range value in the trigger register
/// ***************************************************************************
...
#define DEFAULT_RTC_CONFIG                                  \
    {                                                       \
        .rtc_date_time.time.milliseconds =  0,              \
        ...
        .milliseconds_event_trigger = TR_HAL_EVENT_TRIGGER_OFF, \
