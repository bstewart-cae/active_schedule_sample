...
/// @file tr_hal_pwm.c
///
/// @brief This contains the code for the Trident HAL PWM for T32CM11
///
///
...

{
    ...
    // GPIO must be in range, valid pins for PWM:
    // PWM0: 8,  20, 21, 22, 23
    // PWM1: 9,  20, 21, 22, 23
    // PWM2: 14, 20, 21, 22, 23
    // PWM3: 15, 20, 21, 22, 23
    // PWM4:     20, 21, 22, 23
    uint32_t pin = pwm_settings->pin_to_use.pin;

    // this condition is: if NOT (all valid settings)
    if (!
          (   (pin == 20)
           || (pin == 21)
           || (pin == 22)
           || (pin == 23)
           || (( pwm_id == PWM_0_ID) && (pin == 8))
           || (( pwm_id == PWM_1_ID) && (pin == 9))
           || (( pwm_id == PWM_2_ID) && (pin == 14))
           || (( pwm_id == PWM_3_ID) && (pin == 15))
          )
       )
    {
...

{
    ...
    // GPIO8  can be PWM0 using mode 4 (PWM special)
    // GPIO9  can be PWM1 using mode 4 (PWM special)
    // GPIO14 can be PWM2 using mode 4 (PWM special)
    // GPIO15 can be PWM3 using mode 4 (PWM special)
    // **************************

    bool found_pin = false;

    // GPIO8  can be PWM0 using mode 4
    if ( (pin_num == 8) && (pwm_id == PWM_0_ID) )
    {
        status = tr_hal_gpio_set_mode(pin, TR_HAL_GPIO_MODE_PWM_SPECIAL);
        found_pin = true;
    }

    // GPIO9  can be PWM1 using mode 4
    if ( (pin_num == 9) && (pwm_id == PWM_1_ID) )
    {
        status = tr_hal_gpio_set_mode(pin, TR_HAL_GPIO_MODE_PWM_SPECIAL);
        found_pin = true;
    }

    // GPIO14 can be PWM2 using mode 4
    if ( (pin_num == 14) && (pwm_id == PWM_2_ID) )
    {
        status = tr_hal_gpio_set_mode(pin, TR_HAL_GPIO_MODE_PWM_SPECIAL);
        found_pin = true;
    }

    // GPIO15 can be PWM3 using mode 4
    if ( (pin_num == 15) && (pwm_id == PWM_3_ID) )
    {
        status = tr_hal_gpio_set_mode(pin, TR_HAL_GPIO_MODE_PWM_SPECIAL);
        found_pin = true;
    }

    // if we found it already we are done
    if (found_pin)
    {
        // if it worked, tell the GPIO manager that this pin is in use
        if (status == TR_HAL_SUCCESS)
        {
            tr_hal_gpio_mgr_reserve_gpio(pwm_settings->pin_to_use,
    ...
    // GPIO 20,21,22,23 can be any of the 5 PWMs
    //
    // we already checked if the pins are valid
...
static uint32_t get_clk_div_register_val_from_enum(tr_hal_pwm_clk_div_t clk_div)
{
    switch (clk_div)
    {
        case TR_HAL_PWM_CLOCK_DIVIDER_1:   return PWM_CLK_DIV_1;
        case TR_HAL_PWM_CLOCK_DIVIDER_2:   return PWM_CLK_DIV_2;
        case TR_HAL_PWM_CLOCK_DIVIDER_4:   return PWM_CLK_DIV_4;
        case TR_HAL_PWM_CLOCK_DIVIDER_8:   return PWM_CLK_DIV_8;
        case TR_HAL_PWM_CLOCK_DIVIDER_16:  return PWM_CLK_DIV_16;
        case TR_HAL_PWM_CLOCK_DIVIDER_32:  return PWM_CLK_DIV_32;
        case TR_HAL_PWM_CLOCK_DIVIDER_64:  return PWM_CLK_DIV_64;
        case TR_HAL_PWM_CLOCK_DIVIDER_128: return PWM_CLK_DIV_128;
        case TR_HAL_PWM_CLOCK_DIVIDER_256: return PWM_CLK_DIV_256;
    }
    // should never get here since the clk_div was checked for validity
    // before calling this function
    return PWM_CLK_DIV_1;
}
                                ...
                                tr_hal_pwm_settings_t* pwm_settings)
{
    ...
    // if we got an error on pin set, abort and return that
    if (status != TR_HAL_SUCCESS)
    {
        return status;
    }
    ...
    // the CM11 PWM only has 1 clock source, so we ignore this setting
    // pwm_settings->clock_to_use
    ...
    pwm_register_address->rseq_num_repeats = 1;
