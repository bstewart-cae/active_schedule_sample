...
RFB_INIT_CB rfb_init_cb = rfb_init_default_cb;
...
void rfb_port_fix_tx_power_check(void)
{
    ...
    {
        LOG_DEBUG_PERI("Fix Txpower error: fail to read VBAT\n");
    }
...
RFB_EVENT_STATUS rfb_port_modem_set(rfb_modem_type_t modem, uint8_t band_type)
{
    ...
    case RFB_MODEM_ZIGBEE:
        ...
        {
            LOG_DEBUG_RF("[W] rfb_comm_zigbee_initiate fail, status:%d\n", event_status);
        }
        break;
#endif
    ...
    case RFB_MODEM_FSK:
        ...
        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_fsk_initiate fail, status:%d\n", event_status);
        }
        break;
    case RFB_MODEM_OQPSK:
        ...
        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_oqpsk_initiate fail, status:%d\n", event_status);
        }
        break;
    case RFB_MODEM_ZWAVE:
        /* turn BAND_SUBG into BAND_OQPSK_SUBG */
        if (band_type >= BAND_SUBG_868M)
        {
            event_status = rfb_comm_zwave_initiate((band_type - 1));
        }
        else
        {
            event_status = rfb_comm_zwave_initiate(band_type);
        }
        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_zwave_initiate fail, status:%d\n", event_status);
        }

        event_status = rfb_comm_clock_set(5, band_type, 1); //BLE, 2.4G, 32M clock
        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_clock_set fail, status:%d\n", event_status);
        }
        break;
#endif
...
RFB_EVENT_STATUS rfb_port_subg_init(rfb_interrupt_event_t *_rfb_interrupt_event, rfb_keying_type_t keying_mode, uint8_t band_type)
{
    ...
    else if (keying_mode == RFB_KEYING_ZWAVE)
    {
        event_status = rfb_port_modem_set(RFB_MODEM_ZWAVE, band_type);
        g_rfb.modem_type = RFB_MODEM_ZWAVE;
    }
...
RFB_EVENT_STATUS rfb_port_frequency_set(uint32_t rf_frequency)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_frequency_set fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_wake_on_radio_set(uint32_t rf_frequency, uint16_t rx_on_time, uint32_t sleep_time)
{
    ...
    {
        LOG_DEBUG_RF("Error: rfb_comm_wake_on_radio_set fail, sleep_time %"PRIu32" is too big\n", sleep_time);
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_wake_on_radio_set fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_receive_at(uint8_t rf_channel, uint32_t rx_start_time, uint32_t rx_on_time)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_rx_reserve_set fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_2ch_scan_frequency_set(uint8_t scan_enable, uint32_t rf_freq1, uint32_t rf_freq2)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_2ch_scan_frequency_set fail, status:%d\n", event_status);
    }
...
bool rfb_port_zb_is_channel_free(uint32_t rf_frequency, uint8_t rssi_threshold)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_rssi_read fail, status:%d\n", event_status);
    }
...
bool rfb_port_subg_is_channel_free(uint32_t rf_frequency, uint8_t rssi_threshold)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_rf_idle_set fail, status:%d\n", event_status);
    }
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_rx_enable_set fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_tx_continuous_wave_set(bool tx_enable)
{
    ...
    if (tx_enable)
    {
        ...
        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_single_tone_mode_set fail, status:%d\n", event_status);
        ...
        if (rfb_write_tx_queue_status != RFB_WRITE_TXQ_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_tx_data_send fail, status:%d\n", rfb_write_tx_queue_status);
...
void rfb_port_force_whitening(bool white_en)
{
    uint32_t reg_val;
    uint16_t reg_addr;
...
RFB_EVENT_STATUS rfb_port_tx_continuous_modulated_wave_set(bool tx_enable, bool randomness)
{
    RFB_EVENT_STATUS event_status = RFB_EVENT_SUCCESS;
    ...
    if (tx_enable)
    {
        ...
        rfb_port_force_whitening(randomness);

        event_status = rfb_comm_single_tone_mode_set(3);
        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_single_tone_mode_set fail, status:%d\n", event_status);
        ...
        event_status = rfb_comm_rfe_tx_set(500, 37, 0x0000, 0xffff, 0x00, 0x00, 0x00,
                                           0x00000000, 0x00, 0x0000, 0x0000);
        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_rfe_tx_set fail, status:%d\n", event_status);
    ...
    {
        event_status = rfb_comm_rfe_tx_disable();

        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_rfe_tx_disable fail, status:%d\n", event_status);
...
uint8_t rfb_port_rssi_read(bool *is_rssi_valid)
{
    RFB_EVENT_STATUS event_status = RFB_EVENT_SUCCESS;
    uint8_t rssi;
    event_status = rfb_comm_rssi_read(&rssi);
...
RFB_EVENT_STATUS rfb_port_15p4_address_filter_set(uint8_t mac_promiscuous_mode, uint16_t short_source_address, uint32_t long_source_address_0, uint32_t long_source_address_1, uint16_t pan_id, uint8_t isCoordinator)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_15p4_address_filter_set fail, status:%d\n", event_status);
    }
        ...
        uint32_t mac_max_frame_total_wait_time, uint8_t mac_max_frame_retries, uint8_t mac_min_BE)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_15p4_mac_pib_set fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_15p4_phy_pib_set(uint16_t a_turnaround_time, uint8_t phy_cca_mode, uint8_t phy_cca_threshold, uint16_t phy_cca_duration)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_15p4_phy_pib_set fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_15p4_auto_ack_set(uint8_t auto_ack_enable)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_15p4_auto_ack_set fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_15p4_pending_bit_set(uint8_t pending_bit_enable)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_15p4_pending_bit_set fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_auto_state_set(bool rxOnWhenIdle)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_port_auto_state_set fail, status:%d\n", event_status);
    }
...
uint32_t rfb_port_version_get(void)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_port_version_get fail, status:%d\n", event_status);
    }
    return fwVer;
}
...

{
    ...
    if (g_rfb.modem_type == RFB_MODEM_FSK)
    {
        ...
        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_fsk_modem_set fail, status:%d\n", event_status);
        ...
        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_fsk_mac_set fail, status:%d\n", event_status);
        ...
        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_fsk_preamble_set fail, status:%d\n", event_status);
        ...
        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_fsk_sfd_set fail, status:%d\n", event_status);
        ...
        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_fsk_type_set fail, status:%d\n", event_status);
    ...
    else if (g_rfb.modem_type == RFB_MODEM_OQPSK)
    {
        ...
        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_oqpsk_modem_set fail, status:%d\n", event_status);
        ...
        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_oqpsk_mac_set fail, status:%d\n", event_status);
        ...
        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_oqpsk_preamble_set fail, status:%d\n", event_status);
        }
        */
    }
    else if (g_rfb.modem_type == RFB_MODEM_ZWAVE)
    {
        event_status = rfb_comm_zwave_modem_set(data_rate);
        if (event_status != RFB_EVENT_SUCCESS)
        {
            LOG_DEBUG_RF("[W] rfb_comm_zwave_modem_set fail, status:%d\n", event_status);
...
RFB_EVENT_STATUS rfb_port_sleep_set(void)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_rf_sleep_set fail, status:%d\n", event_status);
    }
...
/* ZWAVE */
RFB_EVENT_STATUS rfb_port_zwave_scan_set(uint8_t scan_mode, uint8_t scan_region)
{
    RFB_EVENT_STATUS event_status = RFB_EVENT_SUCCESS;
    event_status = rfb_comm_zwave_scan_set(scan_mode, scan_region);
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_zwave_scan_set fail, status:%d\n", event_status);
    }

    return event_status;
}

RFB_EVENT_STATUS rfb_port_zwave_id_filter_set(uint32_t home_id, uint16_t node_id, uint8_t home_id_hash)
{
    RFB_EVENT_STATUS event_status = RFB_EVENT_SUCCESS;
    event_status = rfb_comm_zwave_id_filter_set(home_id, node_id, home_id_hash);
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_zwave_id_filter_set fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_ble_modem_set(uint8_t data_rate, uint8_t coded_scheme)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_ble_modem_set fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_ble_mac_set(uint32_t sfd_content, uint8_t whitening_en, uint8_t whitening_init_value, uint32_t crc_init_value)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_ble_mac_set fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_tx_power_set(uint8_t band_type, uint8_t power_index)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_port_tx_power_set fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_tx_power_set_oqpsk(uint8_t band_type, uint8_t power_index)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_port_tx_power_set_oqpsk fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_fix_15dbm_tx_power_set(bool enable, uint8_t band_type)
{
    ...
    {
        LOG_DEBUG_RF("[W] rfb_port_fix_15dbm_tx_power_set fail, unsupported band type: %d\n", band_type);
        return RFB_CNF_EVENT_INVALID_CMD;
    }
    ...
    if (g_fix_power_enable)
    {
        ...
        {
            LOG_DEBUG_RF("Fix Txpower error: unsupported band type: %d\n", g_rfb.modem_type);
...
RFB_EVENT_STATUS rfb_port_15p4_src_addr_match_ctrl(uint8_t ctrl_type)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_port_15p4_src_addr_match_ctrl fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_15p4_short_addr_ctrl(uint8_t ctrl_type, uint8_t *short_addr)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_port_15p4_short_addr_ctrl fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_key_set(uint8_t *key_addr)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_port_key_set fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_csl_receiver_ctrl(uint8_t csl_receiver_ctrl, uint16_t csl_period)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_port_csl_receiver_ctrl fail, status:%d\n", event_status);
    }
...
uint8_t rfb_port_csl_accuracy_get(void)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_port_csl_accuracy_get fail, status:%d\n", event_status);
    }
    return csl_accuracy;
}
...
uint8_t rfb_port_csl_uncertainty_get(void)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_port_csl_uncertainty_get fail, status:%d\n", event_status);
    }
...
RFB_EVENT_STATUS rfb_port_csl_sample_time_update(uint32_t csl_sample_time)
{
    ...
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_port_csl_sample_time_update fail, status:%d\n", event_status);
    }
...
uint32_t rfb_port_rtc_time_read(void)
{
    ...
    {
        LOG_DEBUG_RF("[W] rfb_port_rtc_time_read is in sleep state\n");
        return 0;
    }
#endif
    return rtc_time;
...
void rfb_port_zwave_beam_get(uint8_t *beam_data_address, uint8_t *beam_rssi_addr)
{
    uint32_t temp_data;
    uint8_t page;
    uint16_t q_addr;
    ...
    /* Start from 0x4000 + (page*64), the 11th 4-byte*/
    q_addr = 0x4000 + (page * 64) + (10 * 4);
    RfMcu_MemoryGet(q_addr, beam_data_address, 4);
    *beam_rssi_addr = *beam_data_address;
    *beam_data_address = 0x55;
}

uint8_t rfb_port_zwave_scan_stage_get(void)
{
    uint8_t stage;
    uint32_t temp_data;
    uint8_t page;
    ...
    /* Start from 0x4000 + (page*64), the 12th 4-byte*/
    q_addr = 0x4000 + (page * 64) + (11 * 4);
    RfMcu_MemoryGet(q_addr, (uint8_t *)&temp_data, 4);
    stage = temp_data & 0xff;

    return stage;
}

uint8_t rfb_port_zwave_scan_rssi_read(uint8_t scan_stage, bool *is_rssi_valid, bool average_mode)
{
    RFB_EVENT_STATUS event_status = RFB_EVENT_SUCCESS;
    uint8_t rssi;
    uint32_t temp_data;
    ...
    /* Start from 0x4000 + (page*64), write the 12th 4-byte */
    temp_data = (uint32_t)scan_stage + (uint32_t)(average_mode << 8);
    q_addr = 0x4000 + (page * 64) + (12 * 4);
    RfMcu_MemorySet(q_addr, (uint8_t *)&temp_data, 4);
...
RFB_EVENT_STATUS rfb_port_zwave_all_channel_average_rssi_read(uint8_t *average_rssi)
{
    RFB_EVENT_STATUS event_status = RFB_EVENT_SUCCESS;
    uint8_t average_mode = 1; //get average rssi
    uint8_t scan_stage = 7; //all channels
    uint32_t temp_data;
    uint8_t page;
    uint16_t q_addr;
    ...
    event_status = rfb_comm_zwave_average_rssi_read(average_rssi);

    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_zwave_average_rssi_read fail, status:%d\n", event_status);
    }

    return event_status;
}

RFB_EVENT_STATUS rfb_port_zwave_wake_on_radio_set(uint16_t rx_on_time, uint32_t sleep_time, uint8_t scan_cycle, uint8_t crc_filter,
        uint8_t rssi_read_period, uint8_t rssi_average_number, uint8_t scan_cycle_for_rssi_read)
{
    ...
    /* Start from 0x4000 + (page*64), write the 13th 4-byte */
    temp_data = (uint32_t)scan_cycle + (uint32_t)(crc_filter << 8) + (uint32_t)(rssi_read_period << 16) + (uint32_t)(rssi_average_number << 24);
    q_addr = 0x4000 + (page * 64) + (13 * 4);
    RfMcu_MemorySet(q_addr, (uint8_t *)&temp_data, 4);

    temp_data = (uint32_t)scan_cycle_for_rssi_read;
    q_addr = 0x4000 + (page * 64) + (14 * 4);
    RfMcu_MemorySet(q_addr, (uint8_t *)&temp_data, 4);

    event_status = rfb_comm_wake_on_radio_set(0, rx_on_time, sleep_time);
    if (event_status != RFB_EVENT_SUCCESS)
    {
        LOG_DEBUG_RF("[W] rfb_comm_wake_on_radio_set fail, status:%d\n", event_status);
    }
...
void rfb_port_zwave_scan_rssi_set(uint16_t rssi_read_period, uint8_t rssi_average_number)
{
    uint32_t temp_data;
    uint8_t page;
    uint16_t q_addr;
    ...
    /* Start from 0x4000 + (page*64), write the 13th 4-byte */
    temp_data = (uint32_t)(rssi_read_period << 8) + (uint32_t)(rssi_average_number << 24);
    q_addr = 0x4000 + (page * 64) + (14 * 4);
    RfMcu_MemorySet(q_addr, (uint8_t *)&temp_data, 4);
}

bool rfb_port_init_cb_register(RFB_INIT_CB cb)
{
    if (cb != NULL)
    {
        rfb_init_cb = cb;
