...
void rfb_send_cmd(uint8_t *cmd_address, uint8_t cmd_length)
{
    ...
    {
        LOG_DEBUG_RF("[E] command queue is FULL\n");
    }
}
/**************************************************************************************************
 *    GLOBAL FUNCTIONS
...
void rfb_isr_handler(uint8_t interrupt_status)
{
    ...
    if (RfMcu_PowerStateCheck() != 0x03)
    {
        ...
        LOG_DEBUG_RF("[W] PWR state error in rfb_isr_handler\n");
        return;
    }
    ...
    if (interrupt_state_value.bf.MHR_DONE)
    {
        ...
        // Only for Z-wave: Beam frame RX
        rfb_interrupt_event->rx_beam();
    }

}

void rfb_comm_event_intr_clear(void)
{
    uint16_t intr_status = 0;

    enter_critical_section();

    if (COMM_SUBSYSTEM_AHB->COMM_SUBSYSTEM_INTR_STATUS & RF_MCU_SW_0_INTR)
    {
        intr_status |= RF_MCU_SW_0_INTR;
    }

    if (COMM_SUBSYSTEM_AHB->COMM_SUBSYSTEM_INTR_STATUS & RF_MCU_DMA_DONE_INTR)
    {
        intr_status |= RF_MCU_DMA_DONE_INTR;
    }

    if (intr_status)
    {
        RfMcu_HostWakeUpMcu();
        RfMcu_InterruptClear(intr_status);
        if (__NVIC_GetPendingIRQ(CommSubsystem_IRQn))
        {
            __NVIC_ClearPendingIRQ(CommSubsystem_IRQn);
        }
    }
...
void rfb_comm_event_queue_clear(void)
{
    RfMcu_EventQueueReset();
}

void rfb_comm_rx_queue_read_all(bool rx_queue_commit)
{
    rfb_rx_queue_t *pRxQueue = &g_rx_queue;
    RF_MCU_RXQ_ERROR rxq_status = RF_MCU_RXQ_ERR_INIT;
    uint8_t rx_numIdx;
    ...
    if (rx_queue_commit)
    {
        for (rx_numIdx = 0; rx_numIdx < g_rx_queue.rx_num; rx_numIdx++)
        {
            rfb_interrupt_event->rx_done(pRxQueue->rx_buf[rx_numIdx].rx_control_field.Length,
                                         pRxQueue->rx_buf[rx_numIdx].rx_data,
    ...
    if (COMM_SUBSYSTEM_AHB->COMM_SUBSYSTEM_INTR_STATUS & RF_MCU_BMU_RX_VALID_INTR)
    {
        RfMcu_InterruptClear(RF_MCU_BMU_RX_VALID_INTR);
    }
...
{
    ruci_para_set_wake_on_radio_t sSetWorCmd = {0};
    ruci_para_cnf_event_t sCnfEvent = {0};
    uint8_t event_len = 0;
    RF_MCU_RX_CMDQ_ERROR event_status = RF_MCU_RX_CMDQ_ERR_INIT;

    SET_RUCI_PARA_SET_WAKE_ON_RADIO(&sSetWorCmd, rf_frequency, rx_on_time, sleep_time);

    RUCI_ENDIAN_CONVERT((uint8_t *)&sSetWorCmd, RUCI_SET_WAKE_ON_RADIO);

    enter_critical_section();
    rfb_send_cmd((uint8_t *)&sSetWorCmd, RUCI_LEN_SET_WAKE_ON_RADIO);
    event_status = rfb_event_read(&event_len, (uint8_t *)&sCnfEvent);
    leave_critical_section();
    ...
    if (sCnfEvent.pci_cmd_subheader != RUCI_CODE_SET_WAKE_ON_RADIO)
    {
        return RFB_CNF_EVENT_CONTENT_ERROR;
    }
    if (sCnfEvent.status != RFB_EVENT_SUCCESS)
...
RFB_EVENT_STATUS rfb_comm_rx_reserve_set(uint32_t rx_start_time, uint32_t rx_on_time)
{
    ...
    enter_critical_section();
    rfb_send_cmd((uint8_t *)&sRxReqCmd, RUCI_LEN_SET_RX_RESERVE_ENABLE);
    event_status = rfb_event_read(&event_len, (uint8_t *)&sCnfEvent);
    leave_critical_section();
...
void rfb_comm_fw_preload(rfb_interrupt_event_t *_rfb_interrupt_event)
{
    ...
    rf_common_init_fw_preload(RF_FW_LOAD_SELECT_RUCI_CMD, rfb_isr_handler);
}
...
{
    ruci_para_get_rssi_t sGetRssiCmd = {0};
    ruci_para_get_zwave_rssi_event_t sGetRssiEvent = {0};
    ruci_para_cnf_event_t sCnfEvent = {0};
    uint8_t event_len = 0;
    RF_MCU_RX_CMDQ_ERROR event_status = RF_MCU_RX_CMDQ_ERR_INIT;
    uint8_t get_rssi_event_len = 0;
    ...
    RUCI_ENDIAN_CONVERT((uint8_t *)&sGetRssiEvent, RUCI_GET_ZWAVE_RSSI_EVENT);
    if (get_rssi_event_status != RF_MCU_RX_CMDQ_GET_SUCCESS)
    {
        return RFB_RSP_EVENT_NOT_AVAILABLE;
    }
    if (sGetRssiEvent.sub_header != RUCI_CODE_GET_ZWAVE_RSSI_EVENT)
    {
        return RFB_RSP_EVENT_CONTENT_ERROR;
    }
    rssi_seq[0] = sGetRssiEvent.r3_rssi;
    rssi_seq[1] = sGetRssiEvent.r2_rssi;
    rssi_seq[2] = sGetRssiEvent.r1_rssi;
    rssi_seq[3] = sGetRssiEvent.lr_rssi;
    rssi_seq[4] = sGetRssiEvent.lr_2_rssi;
    rssi_seq[5] = sGetRssiEvent.r3_2_rssi;
    rssi_seq[6] = sGetRssiEvent.r3_3_rssi;
...
RFB_EVENT_STATUS rfb_comm_rfe_tx_enable(ruci_para_set_rfe_tx_enable_t *pRfeTxEnCmd)
{
    ...
    RUCI_ENDIAN_CONVERT((uint8_t *)pRfeTxEnCmd, RUCI_SET_RFE_TX_ENABLE);

    enter_critical_section();
    rfb_send_cmd((uint8_t *)pRfeTxEnCmd, RUCI_LEN_SET_RFE_TX_ENABLE);
    event_status = rfb_event_read(&event_len, (uint8_t *)&sCnfEvent);
    leave_critical_section();
    ...
    if (sCnfEvent.pci_cmd_subheader != RUCI_CODE_SET_RFE_TX_ENABLE)
    {
        return RFB_CNF_EVENT_CONTENT_ERROR;
    }
    if (sCnfEvent.status != RFB_EVENT_SUCCESS)
...
RFB_EVENT_STATUS rfb_comm_rfe_rx_enable(ruci_para_set_rfe_rx_enable_t *pRfeRxEnCmd)
{
    ...
    RUCI_ENDIAN_CONVERT((uint8_t *)pRfeRxEnCmd, RUCI_SET_RFE_RX_ENABLE);

    enter_critical_section();
    rfb_send_cmd((uint8_t *)pRfeRxEnCmd, RUCI_LEN_SET_RFE_RX_ENABLE);
    event_status = rfb_event_read(&event_len, (uint8_t *)&sCnfEvent);
    leave_critical_section();
    ...
    if (sCnfEvent.pci_cmd_subheader != RUCI_CODE_SET_RFE_RX_ENABLE)
    {
        return RFB_CNF_EVENT_CONTENT_ERROR;
    }
    if (sCnfEvent.status != RFB_EVENT_SUCCESS)
...
{
    ruci_para_set_rfe_tx_enable_t sRfeTxSetCmd_t;
    ruci_para_cmn_cnf_event_t sCmnCnfEvent;
    uint8_t event_len = 0;
    RF_MCU_RX_CMDQ_ERROR event_status = RF_MCU_RX_CMDQ_ERR_INIT;

    SET_RUCI_PARA_SET_RFE_TX_ENABLE(&sRfeTxSetCmd_t, pkt_interval, pkt_length, pkt_step_size, pkt_count, pkt_type,
                                    pkt_phr_type, ack_enable, ack_timeout, sec_level, sec_a_data_len, sec_m_data_len);

    RUCI_ENDIAN_CONVERT((uint8_t *)&sRfeTxSetCmd_t, RUCI_SET_RFE_TX_ENABLE);

    enter_critical_section();
    rfb_send_cmd((uint8_t *)&sRfeTxSetCmd_t, RUCI_LEN_SET_RFE_TX_ENABLE);
    event_status = rfb_event_read(&event_len, (uint8_t *)&sCmnCnfEvent);
    leave_critical_section();
    ...
    if (sCmnCnfEvent.cmn_cmd_subheader != RUCI_CODE_SET_RFE_TX_ENABLE)
    {
        return RFB_CNF_EVENT_CONTENT_ERROR;
    }
    if (sCmnCnfEvent.status != RFB_CMN_EVENT_SUCCESS)
...
RFB_EVENT_STATUS rfb_comm_rfe_tx_disable(void)
{
    ruci_para_set_rfe_tx_disable_t sRfeTxDisableCmd_t;
    ruci_para_cmn_cnf_event_t sCmnCnfEvent;
    uint8_t event_len = 0;
    RF_MCU_RX_CMDQ_ERROR event_status = RF_MCU_RX_CMDQ_ERR_INIT;

    SET_RUCI_PARA_SET_RFE_TX_DISABLE(&sRfeTxDisableCmd_t);

    RUCI_ENDIAN_CONVERT((uint8_t *)&sRfeTxDisableCmd_t, RUCI_SET_RFE_TX_DISABLE);

    enter_critical_section();
    rfb_send_cmd((uint8_t *)&sRfeTxDisableCmd_t, RUCI_LEN_SET_RFE_TX_DISABLE);
    event_status = rfb_event_read(&event_len, (uint8_t *)&sCmnCnfEvent);
    leave_critical_section();
    ...
    if (sCmnCnfEvent.cmn_cmd_subheader != RUCI_CODE_SET_RFE_TX_DISABLE)
    {
        return RFB_CNF_EVENT_CONTENT_ERROR;
    }
    if (sCmnCnfEvent.status != RFB_CMN_EVENT_SUCCESS)
